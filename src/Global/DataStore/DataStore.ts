...
interface ParameterlessConstructor<ConstructorType, TDataType, TObserverProps = any>
{
...
export class DataStoreObserver<TDataType, TInputType = any>
{
	...
	public update(newData: TDataType)
	{
		...
		this.updateTimer = window.setTimeout(() => this.callback(newData), this.updateDelayMs);
	}
}

export class DataStore<TDataType extends object, TObserverProps = any, TObserverType extends DataStoreObserver<TDataType, TObserverProps> = DataStoreObserver<TDataType>>
{
	protected _internalState: TDataType = {} as TDataType;
	...
	constructor(
		...
		private readonly subscriptionConstructor: ParameterlessConstructor<TObserverType, TDataType, TObserverProps | undefined> | undefined = undefined,
	 ...
	 * @param data
	 */
	protected update(data: Partial<TDataType>)
	{
	 ...
	 * @param callback
	 */
	public observe(callback: (newData: TDataType) => void, props?: TObserverProps, updateImmediately = true): DestroyCallback
	{
		if (this.propsRequired && props === undefined)
		{
			throw new Error("This data store requires props parameters for each subscription");
		}
